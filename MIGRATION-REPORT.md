...
6. **Product schema carries a fabricated review.** Each `Product` node has
   ...
   Preserved as-is; flagged for your decision — this is the same pattern that
   endangered the zeecustomboxes.com Merchant listings.
7. **The published NAP looks fictional.** Phone `(407) 555-0192` is in the
   ...
   and the header. Preserved exactly; supply the real details and they can be
...
18. **Confirm the real phone number and address** (item 7), or confirm the
    current ones are intended.
...
**Recommended: approve and cut over.** The audit is clean on everything that is
...
Do **not** start the SEO phase until that fortnight is clean — items 1–11 above
are the backlog for it, and every one of them is deliberately untouched today.
